...
import (
	...
	admv1 "gl.king.im/king-lib/framework/test/skeleton/internal/service/admin/v1"
	srvV1 "gl.king.im/king-lib/framework/test/skeleton/internal/service/service/v1"
...
func NewServer(skeletonSvc *srvV1.SkeletonService, skeletonGinSvc *srvV1.SkeletonGinService, skeletonAdmSvc *admv1.SkeletonAdminService, sche scheduler.DistributedScheduler, logger log.Logger) *boot.NewServerCollection {
	...
	sc = append(sc, boot.ServerRegisterCnf{
		...
		Http:        pb.RegisterSkeletonServiceHTTPServer,
		Grpc:        pb.RegisterSkeletonServiceServer,
		FileDesp:    pb.File_api_skeleton_service_v1_skeleton_service_proto,
		ServiceDesp: protodesc.ToServiceDescriptorProto(pb.File_api_skeleton_service_v1_skeleton_service_proto.Services().ByName("SkeletonService")),
	})
